...
<body class="p-3 p-lg-5">
        ...
        <form id="frm" class="d-flex flex-column gap-3" enctype="multipart/form-data">
          ...
          <div class="d-flex gap-2">
            <button id="btnSubir" type="submit" class="btn btn-acento">
              <i class="bi bi-cloud-upload"></i> Subir e importar
            </button>
  ...
  const ENDPOINT = location.pathname; // mismo PHP
  ...
  frm.addEventListener('submit', async (e)=>{
    ...
    try{
      ...
      if (data.errores && data.errores.length) {
        const muestra = data.errores.slice(0, 20).map(e => '- ' + e).join('\n');
        errores = `\nErrores (primeros 20):\n${muestra}`;
      }
      log.textContent =
        `✅ Importación completada\nInsertados: ${data.insertados}\nSaltados: ${data.saltados}${errores}`;
    }catch(err){
      if (err.name === 'AbortError'){
        log.textContent = '⛔ Importación cancelada por el usuario.';
      }else{
...
// si el cliente cierra la conexión, detenemos (para soportar Cancelar)
...
function norm_bigint_text(?string $s): ?string {
  ...
  if (preg_match('/^(\d+(?:\.\d+)?)e\+?(-?\d+)$/i', $s, $m)) {
    ...
    if ($exp >= strlen($frac)) $digits = $int.$frac.str_repeat('0', $exp - strlen($frac));
    else $digits = $int.substr($frac,0,$exp);
...
$counts = ["\t"=>substr_count($firstLineRaw,"\t"), ";"=>substr_count($firstLineRaw,";"), ","=>substr_count($firstLineRaw,",")];
arsort($counts);
...
// ===== Insert fila a fila (SAVEPOINT) =====
$insertados=0; $saltados=0; $errores=[]; $linea=1;
$pdo->beginTransaction();
while (($line = fgets($fh)) !== false) {
  $linea++;
  if (connection_aborted()) { break; } // usuario canceló
