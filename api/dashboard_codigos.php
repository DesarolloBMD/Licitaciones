...
/* ---- 1) Catálogo filtrado a asesoría / imprenta ----
 Devuelve: [ {codigo, descripcion, tipo, total_pa} ... ]
*/
if ($accion === 'catalogo') {
  $q = $pdo->query(
    'SELECT
        ...
        COALESCE((
          SELECT COUNT(*) FROM public."Procedimientos Adjudicados" p
           WHERE p."CODIGO_MADRE" = c."Codigo de Clasificación"
        ),0) AS total_pa
...
Devuelve KPIs, top proveedores, top instituciones, serie mensual, recientes
*/
if ($accion === 'stats') {
  $codigo = $_GET['codigo'] ?? '';
  ...
  ');
  $stmt->execute([':cod'=>$codigo]);
  $topInst = $stmt->fetchAll();

  // SERIE MENSUAL (por "Mes de Descarga")
  $stmt = $pdo->prepare('
    WITH base AS (
      SELECT *
        FROM public."Procedimientos Adjudicados"
