...
require_once 'https://cdn.jsdelivr.net/gh/PHPOffice/PhpSpreadsheet@1.29.0/src/PhpSpreadsheet/IOFactory.php';
...
function normalizar_fecha($v){ 
  if (!$v) return null;
  if (is_numeric($v)) return date('Y-m-d', \PhpOffice\PhpSpreadsheet\Shared\Date::excelToTimestamp($v));
...
if ($ext !== 'xlsx'){
  echo json_encode(['ok'=>false,'error'=>'Solo se permite formato .xlsx (Excel)']);
  exit;
}
